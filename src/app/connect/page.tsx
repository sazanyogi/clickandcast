...
function LinkButton({ href, label, icon, external, delay }: LinkItem & { delay: number }) {
  return (
    <motion.a
      ...
      style={{ borderColor: "rgba(65,65,65,0.7)", backgroundColor: "rgba(255,255,255,0.02)", color: "#b0b0b0" }}
      onMouseEnter={(e) => {
      ...
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLElement).style.borderColor = "rgba(65,65,65,0.7)";
    ...
    >
      <span style={{ color: "#E8174D" }}>{icon}</span>
      <span className="flex-1">{label}</span>
      <svg className="opacity-25 group-hover:opacity-60 transition-opacity shrink-0" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
/* ─── section card ─── */
function Section({ title, role, links, baseDelay }: { title: string; role: string; links: LinkItem[]; baseDelay: number }) {
  return (
    <motion.div
      ...
      style={{ borderColor: "rgba(65,65,65,0.7)" }}
    >
      {/* Section header */}
      <div className="px-5 py-4 border-b" style={{ borderColor: "rgba(65,65,65,0.7)", backgroundColor: "rgba(255,255,255,0.03)" }}>
        <p className="text-xs font-semibold uppercase tracking-[1.4px] mb-0.5" style={{ color: "#E8174D" }}>{role}</p>
        <h2 className="text-sm font-black text-white">{title}</h2>
      </div>
      {/* Links */}
      <div className="p-3 space-y-1.5">
        {links.map((link, i) => (
          <LinkButton key={link.label} {...link} delay={baseDelay + 0.08 + i * 0.05} />
        ))}
      </div>
    </motion.div>
  );
...
export default function ConnectPage() {
  ...
  const spotlight = useMotionTemplate`radial-gradient(500px circle at ${x}px ${y}px, rgba(232,23,77,0.10), transparent 70%)`;
  ...
  return (
    <div className="relative min-h-screen bg-black text-white overflow-hidden">
      ...
      <div className="pointer-events-none absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] rounded-full blur-[120px] opacity-20" style={{ backgroundColor: "#E8174D" }} />
      ...
      <div className="relative z-10 flex flex-col items-center px-4 pt-16 pb-16">
        ...
        >
          ...
          <motion.h1
            ...
            style={{ fontSize: "clamp(2.4rem, 10vw, 3.2rem)", fontWeight: 900 }}
          >
            Where Stories<br />
            <span style={{ color: "#E8174D" }}>Come Alive.</span>
          </motion.h1>
          ...
          <motion.div
            ...
            style={{ backgroundColor: "#E8174D", transformOrigin: "center" }}
          />
        </motion.div>
        ...
        <motion.p
          ...
          style={{ color: "#333333" }}
        >
          © {new Date().getFullYear()} Click & Cast Inc.
        </motion.p>
      </div>
    </div>
  );
